...
@section('page-title', 'Nova Eleição')
...
<div class="card" style="max-width: 600px;">
    <div class="card-body">
        <form method="POST" action="{{ route('admin.eleicoes.store') }}">
            @csrf
                ...
                @forelse($cidades as $cidade)
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox"
                            ...
                            {{ in_array($cidade->id, old('cidades', [])) ? 'checked' : '' }}>
                        <label class="form-check-label" for="cidade_{{ $cidade->id }}">
                            {{ $cidade->nome }}
                        </label>
                    </div>
                @empty
                    <p class="text-muted small">Nenhuma missão cadastrada.
                        <a href="{{ route('admin.cidades.create') }}">Cadastrar missão</a>
                    </p>
                @endforelse
            </div>
