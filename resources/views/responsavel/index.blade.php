...
@forelse($eleicoes as $eleicao)
    ...
    <div class="card eleicao-card mb-4">
        ...
        <div class="card-body p-0">
            ...
            @if($temVida)
            <div class="p-4 {{ $temAlianca ? 'border-bottom' : '' }}">
                <div class="d-flex justify-content-between align-items-start flex-wrap gap-3">
                    <div>
                        ...
                        <p class="text-muted small mb-0">Votação única — válida para todas as cidades simultaneamente.</p>
                        ...
                        {{-- Membros Vida por cidade (quando não há aliança) --}}
                        @if(!$temAlianca)
                            <div class="d-flex gap-4 mt-2">
                                @foreach($eleicao->cidades as $ec)
                                <div>
                                    @if(auth()->user()->perfil === 'admin')
                                        <div class="resp-metric-label">{{ $ec->cidade->nome }}</div>
                                    @else
                                        <div class="resp-metric-label">Membros Vida</div>
                                    @endif
                                    <div class="resp-metric-value">{{ $ec->qtd_vida > 0 ? $ec->qtd_vida : '—' }}</div>
                                </div>
                                @endforeach
                            </div>
                        @endif
                    ...
                    <div class="d-flex flex-column gap-2 align-items-end">
                        {{-- Configurar membros vida quando não há aliança --}}
                        @if(!$temAlianca)
                            @foreach($eleicao->cidades as $ec)
                        ...
                        @if(!$eleicao->aberta_vida && !$eleicao->data_encerramento_vida)
                            <a href="{{ route('responsavel.vida.abrir', $eleicao) }}" class="btn btn-resp-primario">
                                <i class="bi bi-play-circle me-1"></i>Abrir Votação Vida
                            </a>
                        @endif
                        @if($eleicao->aberta_vida)
                    ...
                    <div class="d-flex justify-content-between align-items-start flex-wrap gap-3">
                        <div class="flex-grow-1">
                            ...
                            <div class="d-flex gap-4 mt-2">
                                @if($temVida)
                                <div>
                                    <div class="resp-metric-label">Vida (membros)</div>
